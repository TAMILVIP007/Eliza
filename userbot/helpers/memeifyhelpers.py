...
async def per_meme(MY_FONTS, topString, bottomString, filename, endname):
    img = Image.open(filename)
    imageSize = img.size
    # find biggest font size that works
    fontSize = int(imageSize[1] / 5)
    font = ImageFont.truetype(MY_FONTS, fontSize)
...
async def per_meeme(upper_text, lower_text, MY_FONTS, picture_name, endname):
    ...
    text_draw = Drawing()
    text_draw.font = join(getcwd(), MY_FONTS)
